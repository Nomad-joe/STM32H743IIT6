...
uint8_t sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    HAL_PWREx_ConfigSupply(PWR_LDO_SUPPLY);                         /* LDOEN = 1, 使能LDO */
 
    __HAL_RCC_SYSCFG_CLK_ENABLE();                                  /* 使能SYSCFG外设时钟 */
    __HAL_PWR_VOLTAGESCALING_CONFIG(PWR_REGULATOR_VOLTAGE_SCALE0);  /* 使能Overdrive模式, 电压调节为Scale0, 1.35V内核电压, 使用最高主频480MHz */
