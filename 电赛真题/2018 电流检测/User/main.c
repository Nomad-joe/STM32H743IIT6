...
//    arm_cmplx_mag_f32(FFT_input, FFT_mag, FFT_LEN);
  ...
  //  for(uint16_t i=0;i<FFT_LEN/2;i++){printf("%.3f\n",FFT_mag[i]);} 
	 ...
	 Finally_Freq = Finally_Index*0.732421875;
	 Finally_Freq=Finally_Freq/1000;
			
   Finally_Amp = sqrt(FFT_mag[Index-2] * FFT_mag[Index-2] +FFT_mag[Index-1] * FFT_mag[Index-1] + FFT_mag[Index] * FFT_mag[Index] + FFT_mag[Index+1] * FFT_mag[Index+1] +FFT_mag[Index+2] * FFT_mag[Index+2])/sqrt(5)*1.859;
   ...
   Finally_Amp_2 = sqrt(FFT_mag[Index_2-2] * FFT_mag[Index_2-2] + FFT_mag[Index_2-1] * FFT_mag[Index_2-1] + FFT_mag[Index_2] * FFT_mag[Index_2] + FFT_mag[Index_2+1] * FFT_mag[Index_2+1] +FFT_mag[Index_2+2] * FFT_mag[Index_2+2]) /sqrt(5)*1.89;
	  Finally_Amp_3 = sqrt(FFT_mag[Index_3-2] * FFT_mag[Index_3-2] + FFT_mag[Index_3-1] * FFT_mag[Index_3-1] + FFT_mag[Index_3] * FFT_mag[Index_3] + FFT_mag[Index_3+1] * FFT_mag[Index_3+1] +FFT_mag[Index_3+2] * FFT_mag[Index_3+2]) /sqrt(5)*1.88;
		 Finally_Amp_4 = sqrt(FFT_mag[Index_4-2] * FFT_mag[Index_4-2] + FFT_mag[Index_4-1] * FFT_mag[Index_4-1] + FFT_mag[Index_4] * FFT_mag[Index_4] + FFT_mag[Index_4+1] * FFT_mag[Index_4+1] +FFT_mag[Index_4+2] * FFT_mag[Index_4+2]) /sqrt(5)*1.87;
		  Finally_Amp_5 = sqrt(FFT_mag[Index_5-2] * FFT_mag[Index_5-2] + FFT_mag[Index_5-1] * FFT_mag[Index_5-1] + FFT_mag[Index_5] * FFT_mag[Index_5] + FFT_mag[Index_5+1] * FFT_mag[Index_5+1] +FFT_mag[Index_5+2] * FFT_mag[Index_5+2]) /sqrt(5)*1.87;
