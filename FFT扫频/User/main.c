...
// 拟合输出：归一化分母形式 G(s)=(B0 s2+B1 s+B2)/(s2+A1 s+A2)
typedef struct
{
    double A1;
    double A2;
...
void Scan_Freq(float Start_F, float End_F,BodeData* data);
int main(void)
{  
 //   sys_cache_enable();                     /* 使能L1-Cache */
  ...
  HAL_TIM_Base_Start(&htim3);
	AD9834_AmpSet(90);//设置Vpp = 1V
	  AD9834_WaveSeting(100,0,1,0);
//	 for(uint16_t i=100;i<50000;i+=100)
//	 {
...
//	 }
   // 1.扫频
    ...
    for (int i = 0; i < 5; i++) {
        ...
        // 计算 G(jw) = (B0*(jw)^2 + B1*(jw) + B2) / ((jw)^2 + A1*(jw) + A2)
        // 分子：(-B0*w2 + B2) + j(B1*w)
        // 分母：(-w2 + A2) + j(A1*w)
...
//		{
//			printf("测量频点:%.2f hz   ",data[i].freq);
//			printf("增益:%.2f db   ",data[i].gain_db);
//			printf("相位差:%.2f 度\r\n",data[i].phase_deg);
//		}
    while (1)
    {
...
void Scan_Freq(float Start_F, float End_F,BodeData* data)  //输入参数：开始频率，终止频率，存放增益数组，存放相位差数组   
{
  ...
  for(uint16_t DDS_freq=Start_F;DDS_freq<=End_F;DDS_freq+=100){
   ...
   //   printf("放大倍数:%.2f\r\n",1.8*FFT_Amp_2/Finally_Amp);
			data[point].gain_db = 20.0 * log10(1.8*FFT_Amp_2/Finally_Amp); 
			
		// 计算实部的权重（用实部平方）
float R1_prev = FFT_input[2*(Index-1)] * FFT_input[2*(Index-1)];
float R1_curr = FFT_input[2*Index]     * FFT_input[2*Index];
float R1_next = FFT_input[2*(Index+1)] * FFT_input[2*(Index+1)];
float real_1 = (FFT_input[2*(Index-1)] * R1_prev + 
                FFT_input[2*Index]     * R1_curr + 
                FFT_input[2*(Index+1)] * R1_next) / (R1_prev + R1_curr + R1_next);

// 计算虚部的权重（用虚部平方）
float I1_prev = FFT_input[2*(Index-1)+1] * FFT_input[2*(Index-1)+1];
float I1_curr = FFT_input[2*Index+1]     * FFT_input[2*Index+1];
float I1_next = FFT_input[2*(Index+1)+1] * FFT_input[2*(Index+1)+1];
float img_1 = (FFT_input[2*(Index-1)+1] * I1_prev + 
               FFT_input[2*Index+1]     * I1_curr + 
               FFT_input[2*(Index+1)+1] * I1_next) / (I1_prev + I1_curr + I1_next);

// 同理 for FFT_input_2
float R2_prev = FFT_input_2[2*(Index-1)] * FFT_input_2[2*(Index-1)];
float R2_curr = FFT_input_2[2*Index]     * FFT_input_2[2*Index];
float R2_next = FFT_input_2[2*(Index+1)] * FFT_input_2[2*(Index+1)];
float real_2 = (FFT_input_2[2*(Index-1)] * R2_prev + 
                FFT_input_2[2*Index]     * R2_curr + 
                FFT_input_2[2*(Index+1)] * R2_next) / (R2_prev + R2_curr + R2_next);

float I2_prev = FFT_input_2[2*(Index-1)+1] * FFT_input_2[2*(Index-1)+1];
float I2_curr = FFT_input_2[2*Index+1]     * FFT_input_2[2*Index+1];
float I2_next = FFT_input_2[2*(Index+1)+1] * FFT_input_2[2*(Index+1)+1];
float img_2 = (FFT_input_2[2*(Index-1)+1] * I2_prev + 
               FFT_input_2[2*Index+1]     * I2_curr + 
               FFT_input_2[2*(Index+1)+1] * I2_next) / (I2_prev + I2_curr + I2_next);

// 计算相位
float phase_1 = atan2f(img_1, real_1) * 180.0f / PI;
float phase_2 = atan2f(img_2, real_2) * 180.0f / PI;
