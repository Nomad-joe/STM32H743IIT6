...
float b0 = 72.7447128296f;
float b1 = -147.2697906494f;
float b2 = 75.3759994507f;
float a1 = -2.7663781643f;
float a2 = 2.6278600693f;

// 滤波后的数据缓冲区
uint16_t Filtered_Buffer[DMA_Len];

// ADC转电压
#define ADC_TO_VOLT(adc)  (((float32_t)(adc) / 4095.0f * 3.3f - 1.65f) / 1.65f)/1.6f
...
//			 for(uint16_t i=0;i< DMA_Len/2;i++)
//			 {  printf("%d\r\n",DMA_Buffer[i]);
//			 }
...
//		  for(uint16_t i=DMA_Len/2;i< DMA_Len;i++)
//			 {    printf("%df\r\n",DMA_Buffer[i]);
//			 }
