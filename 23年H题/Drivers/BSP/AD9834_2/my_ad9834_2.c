#include "./BSP/AD9834_2/my_ad9834_2.h"
  ...
  #define PORT_CS		GPIOD
	#define PIN_CS		GPIO_PIN_4  //数字电位器片选
...
*********************************************************************************************************
*/
static void AD9834_Delay_2(void)
{
	unsigned int i;
	for (i = 0; i < 10; i++);
}
...
/****************************************************************
函数名: AD9834_Write_16Bits_2
功能: 向AD9834写入16位数据
参数: data --  要写入的16位数据
返回值: 无
*****************************************************************/
void AD9834_Write_16Bits_2(unsigned int Data)
{
    unsigned char i = 0 ;
...
返回值: 无
************************************************************************************/
void AD9834_Select_Wave_2(unsigned int initdata)
{
    FSYNC_1();
    SCK_1();
    ...
    RESET_1();
    RESET_0();
    //AD9834_Write_16Bits_2(0x0100); //复位
    AD9834_Write_16Bits_2(initdata);
}

void AD9834_SetFSK_2(unsigned long Freq1,unsigned long Freq2)//正弦波频率调制
{
    FSYNC_1();
    SCK_1();
    ...
    RESET_0();
    AD9834_Write_16Bits_2(0x0100); //复位
    AD9834_Write_16Bits_2(0x2228);
	  AD9834_Set_Freq_2(FREQ_0, Freq1);
	  AD9834_Set_Freq_2(FREQ_1, Freq2);
}

void AD9834_SetPSK_2(float Phase1,float Phase2,unsigned long Freq)//正弦波相位调制
{
    FSYNC_1();
    SCK_1();
    ...
    AD9834_Write_16Bits_2(0x0100); //复位
    AD9834_Write_16Bits_2(0x2228);
	  AD9834_Set_Phase_2(0,Phase1);
	  AD9834_Set_Phase_2(1,Phase2);
	  AD9834_Set_Freq_2(FREQ_0, Freq);
	  AD9834_Set_Freq_2(FREQ_1, Freq);
}
/****************************************************************
函数名: AD9834_Set_Freq_2
功能: 设置频率值
参数: freq_chanel -- 要写入的频率寄存器(FREQ_0或FREQ_1)
          freq -- 频率值 (Freq_value(value)=Freq_data(data)*FCLK/2^28)
返回值: 无
*****************************************************************/
void AD9834_Set_Freq_2(unsigned char freq_chanel, unsigned long freq)
{
    ...
    AD9834_Write_16Bits_2(FREQREG_LSB_14BIT);
    AD9834_Write_16Bits_2(FREQREG_MSB_14BIT);
    
}

/*****************************
函数名: AD9834_Set_Phase_2
功能: 设置相位值
参数: CH -- 要写入的相位寄存器(FREQ_0或FREQ_1)
          Phase --相位值 0-360，12位数据，4096/360=.....
返回值: 无
*****************************/
void AD9834_Set_Phase_2(unsigned char CH,float Phase)
{ 
 //   PS = PS * 40.96 / 3.60 + 0.3;
	Phase = Phase * 11.3777778 + 0.3;	
	
	if(CH==0)
	    AD9834_Write_16Bits_2((unsigned int)Phase + 0xc000);
	else
      AD9834_Write_16Bits_2((unsigned int)Phase + 0xe000);
}
...
void AD9834_AmpSet_2(unsigned char amp)
{
	unsigned char i;
	unsigned int temp;
...
*********************************************************************************************************
*/ 
void AD9834_WaveSeting_2(double Freq,unsigned int Freq_SFR,unsigned int WaveMode,unsigned int Phase )
{
...
//		AD9834_Write_16Bits_2(0x0100); //复位AD9833,即RESET位为1
//		AD9834_Write_16Bits_2(0x2100); //选择数据一次写入，B28位和RESET位为1
		...
		if(Freq_SFR==0)				  //把数据设置到设置频率寄存器0
		{
			Phs_data=Phase|0xC000;	//相位值
			 ...
			 //使用频率寄存器0输出波形
			AD9834_Write_16Bits_2(frequence_LSB); //L14，选择频率寄存器0的低14位数据输入
			AD9834_Write_16Bits_2(frequence_MSB); //H14 频率寄存器的高14位数据输入
			AD9834_Write_16Bits_2(Phs_data);	//设置相位
		...
		if(Freq_SFR==1)				//把数据设置到设置频率寄存器1
		{
			...
			//使用频率寄存器1输出波形
			AD9834_Write_16Bits_2(frequence_LSB); //L14，选择频率寄存器1的低14位输入
			AD9834_Write_16Bits_2(frequence_MSB); //H14 频率寄存器1为
			AD9834_Write_16Bits_2(Phs_data);	//设置相位
			//AD9833_Write(0x2800); /**设置FSELECT位为0，设置FSELECT位为1，即使用频率寄存器1的值，芯片进入工作状态,频率寄存器1输出波形**/
		}

		if(WaveMode==TRI_WAVE) //输出三角波波形
		 	AD9834_Write_16Bits_2(0x2002); 
		if(WaveMode==SQU_WAVE)	//输出方波波形
			AD9834_Write_16Bits_2(0x2028); 
		if(WaveMode==SIN_WAVE)	//输出正弦波形
			AD9834_Write_16Bits_2(0x2000); 
