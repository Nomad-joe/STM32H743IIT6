...
本仓库汇集了基于 **STM32H743IIT6** 高性能微控制器的一系列嵌入式工程，覆盖 **信号采集、信号发生、数字信号处理（DSP）、闭环控制、频率/相位测量** 等方向，是准备电子设计竞赛（电赛）过程中沉淀的完整代码库。

芯片基于 **ARM Cortex-M7** 内核，主频 **480MHz**，内置双精度浮点单元（FPU）与 DSP 指令集，非常适合高速 ADC 采样、FFT 频谱分析、数字滤波等实时信号处理场景。

> 💡 每个子工程均为独立的 Keil MDK 工程，可直接编译、下载、验证。
...
| **信号发生** | AD9833 / AD9910（DDS 芯片）、软件 DDS、DAC 任意波形输出、DAC 钢琴音色合成 |
| **数字信号处理** | 基 4 FFT、加窗（Hann）、FIR / IIR 滤波、最小二乘法拟合、谐波分析（基波/二次/三次） |
| **闭环控制** | PID 锁相环、方波相位差 PID 调节 |
| **测量** | 定时器输入捕获测频、输入捕获 + DMA 测频、FFT 鉴相、李萨如（Lissajous）图形相位差测量 |
| **显示与通信** | RGB-LCD（LTDC）波形/图形显示、UART 串口通信 |
...
├── AD7606FFT相位差李沙育图/      # 相位差测量 + 李萨如图形显示
...
├── 音乐/                         # 音乐播放
└── 电赛真题/
    ├── 2018 电流检测/            # 2018 年电赛真题
    └── 2021归一化幅值/           # 2021 年电赛真题
```
...
| 题目 | 年份 | 说明 |
|------|------|------|
| **电流检测** | 2018 | 基于 ADC + DMA 采集，FFT 频谱分析实现电流信号参数测量 |
| **归一化幅值** | 2021 | 加窗 FFT + 谐波分析（基波/二次/三次谐波），幅值归一化测量 |
